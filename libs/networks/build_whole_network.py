...
class DetectionNetwork(object):
        ...
        with tf.name_scope('postprocess_rfcn'):
            ...
            for i in range(1, cfgs.CLASS_NUM + 1):
                ...
                keep = tf.image.non_max_suppression(
                    ...
                    scores=tmp_score,
                    max_output_size=cfgs.RFCN_NMS_MAX_BOXES_PER_CLASS,
                    iou_threshold=cfgs.RFCN_NMS_IOU_THRESHOLD)
    ...
    def build_rfcn_head(self, rfcn_base, rois, img_shape, bin_nums, crop_size):

        with tf.variable_scope('RFCN'):
            # 5. PSROI Pooling
            conv_new_1 = slim.conv2d(rfcn_base, 1024, [1, 1], trainable=self.is_training,
                                     weights_initializer=cfgs.INITIALIZER, activation_fn=tf.nn.relu, scope='conv_new1')

            rfcn_cls = slim.conv2d(conv_new_1, bin_nums[0] * bin_nums[1] * (cfgs.CLASS_NUM + 1), [1, 1],
                                   trainable=self.is_training,
                                   weights_initializer=cfgs.INITIALIZER,
                                   activation_fn=None, scope='rfcn_cls')
            rfcn_bbox = slim.conv2d(conv_new_1, bin_nums[0] * bin_nums[1] * 4 * (cfgs.CLASS_NUM + 1), [1, 1],
                                    trainable=self.is_training,
                                    weights_initializer=cfgs.INITIALIZER,
                                    activation_fn=None, scope='rfcn_bbox')
    ...
    #         # 5. PSROI Pooling
    #         initializer = tf.random_normal_initializer(mean=0.0, stddev=0.01)
    #         conv_new_1 = slim.conv2d(rfcn_base, 256, [1, 1], trainable=self.is_training,
    #                                  weights_initializer=initializer, activation_fn=tf.nn.relu, scope='conv_new1')
    #
    #         rfcn_cls = slim.conv2d(conv_new_1, bin_nums[0] * bin_nums[1] * (cfgs.CLASS_NUM + 1), [1, 1],
    #                                trainable=self.is_training,
    ...
    #                                        num_spatial_bins=bin_nums)
    #
    #         bbox_pred = self.psroi_pooling(feature_maps=rfcn_bbox,
    ...
    def build_whole_detection_network(self, input_img_batch, gtboxes_batch):
        ...
        with tf.variable_scope('build_rpn',
            ...
            rpn_conv3x3 = slim.conv2d(rpn_base, 512, [3, 3],
                                      trainable=self.is_training,
                                      weights_initializer=cfgs.INITIALIZER,

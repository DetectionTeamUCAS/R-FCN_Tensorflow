...
def proposal_target_layer(rpn_rois, gt_boxes):
    ...
    rois_per_image = np.inf if cfgs.RFCN_MINIBATCH_SIZE == -1 else cfgs.RFCN_MINIBATCH_SIZE

    fg_rois_per_image = np.round(cfgs.RFCN_POSITIVE_RATE * rois_per_image)
...
def _sample_rois(all_rois, gt_boxes, fg_rois_per_image,
    ...
    fg_inds = np.where(max_overlaps >= cfgs.RFCN_IOU_POSITIVE_THRESHOLD)[0]

    # Guard against the case when an image has fewer than fg_rois_per_image
    # Select background RoIs as those within [BG_THRESH_LO, BG_THRESH_HI)
    bg_inds = np.where((max_overlaps < cfgs.RFCN_IOU_POSITIVE_THRESHOLD) &
                       (max_overlaps >= cfgs.RFCN_IOU_NEGATIVE_THRESHOLD))[0]
